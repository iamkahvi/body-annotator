...
## What this app is
A single‑page, static web app for tracking body‑part notes. Users click a body part, enter a description plus start/end dates, and the app stores notes in `localStorage`. Notes are listed with search/sort/filter and the body graphic is highlighted based on recency.
...
- `app.js`: application state + UI wiring.
  ...
  - Opens/closes modal, creates/edits/deletes notes and their date ranges.
...
Body parts are colored from base gray to accent based on the most recent end date in the last ~21 days. More recent end dates = stronger highlight.
